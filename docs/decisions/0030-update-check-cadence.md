...
review toolbar and should reuse the same updater scheduler so multiple triggers
do not overlap network requests.
...
- Long-running app sessions can discover new releases without requiring a
  restart.
- Users can explicitly check for updates while reviewing a file.
